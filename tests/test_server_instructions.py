...
def test_robot_spec_is_merged_into_initial_server_instructions(monkeypatch) -> None:
    ...
    assert "before moving to establish the starting pose" in instructions
    assert "one appropriately sized corrective motion" in instructions
    assert "odometry once more before reporting" in instructions
...
def test_contoro_verified_spec_exposes_operational_topics() -> None:
    ...
    assert "starting pose" in spec["prompts"]
    assert "corrective motion" in spec["prompts"]
    assert "final verification before reporting" in spec["prompts"]
