...
def register_topic_tools(
    ...
    @mcp.tool(
        description=(
            ...
            "subscribe_once returns one current snapshot and then unsubscribes. Call it again after motion "
            "when a fresh observation is required. Image topics return MCP image content that vision-capable "
            "clients can inspect.\n"
    ...
    @mcp.tool(
        description=(
            ...
            "(useful for controllers like diff_drive that need continuous commands). For cmd_vel motion, "
            "use the robot specification's required rate and explicitly publish a zero-velocity message at "
            "the end; waiting after a non-zero message is not itself a stop command.\n"
